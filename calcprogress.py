...
def load_progress_info(ctx: c.SourceContext, asm_list: str
    ...
    raw = c.get_cmd_stdout(
        f"{c.PROGRESS} {ctx.binary} {ctx.labels} {ctx.slices}")
    dat = json.loads(raw)
    assert dat.get(
        "version") == 2, "Outdated progress json version, try a clean & rebuild"
...
def print_binary_progress(sections: List[str], decomp_sizes: Dict[str, int],
    ...
    table = PrettyTable(["Section", "Decompiled Bytes",
                        "Total Bytes", "Percentage"])
    bin_decomp_size = 0
    bin_total_size = 0
    def pct(d, t): return f"{(d/t)*100:.4f}"
    for sec in sections:
...
if __name__ == "__main__":
    ...
    parser.add_argument("-f", "--full", action="store_true",
                        help="Print progress of all sections")
    ...
    print_binary_progress(dol_secs, decomp_sizes,
                          total_sizes, "main.dol", args.full)
